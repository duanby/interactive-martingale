...
# L:     the length of sequence to obtain the upper bound for
# alpha: Type 1 error level
...
ub_func_curve_gamma <- function(k, alpha){
  ...
  ub_scaled = k1*sqrt(k*l) + c*k2*l 
  ub_k = ub_scaled*2*sqrt(2) 
  return(ub_k)
}


# an empirical bound for sub-Exponential distribution
# h_var:   sample variance of the current sum of h(P)
# m:       parameter of the upper bound that change the tight region
# ub_para: parameter to determine the bound obtained by ub_exp_para
